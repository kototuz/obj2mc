...
pub fn Quaternion(comptime T: type) type {
    ...
    return extern struct {
        ...
        pub fn lookAlong(self: Self, dir: Vector3, up: Vector3) Self {
            // Normalize direction
            const invDirLength = Self.invsqrt(dir.x() * dir.x() + dir.y() * dir.y() + dir.z() * dir.z());
            const dirnX = -dir.x() * invDirLength;
            const dirnY = -dir.y() * invDirLength;
            const dirnZ = -dir.z() * invDirLength;
            // left = up x dir
            var leftX = up.y() * dirnZ - up.z() * dirnY;
            var leftY = up.z() * dirnX - up.x() * dirnZ;
            var leftZ = up.x() * dirnY - up.y() * dirnX;
            // normalize left
            const invLeftLength = Self.invsqrt(leftX * leftX + leftY * leftY + leftZ * leftZ);
            leftX *= invLeftLength;
            leftY *= invLeftLength;
            leftZ *= invLeftLength;
            // up = direction x left
            ...
            // invert
            const a = Self.fma(-leftX, upnY, leftY * upnX);
            const b = Self.fma(-leftZ, upnX, leftX * upnZ);
            const c = Self.fma(-leftY, upnZ, leftZ * upnY);
            const d = Self.fma(a, -dirnZ, Self.fma(b, -dirnY, c * -dirnX));
            const s = 1.0 / d;
            const nm00 = Self.fma(upnY, -dirnZ, dirnY * upnZ) * s;
            const nm01 = Self.fma(-dirnY, -leftZ, leftY * -dirnZ) * s;
            const nm02 = c * s;
            const nm10 = Self.fma(-dirnX, upnZ, -upnX * -dirnZ) * s;
            const nm11 = Self.fma(-leftX, -dirnZ, dirnX * -leftZ) * s;
            const nm12 = b * s;
            const nm20 = Self.fma(upnX, -dirnY, dirnX * upnY) * s;
            const nm21 = Self.fma(-dirnX, -leftY, leftX * -dirnY) * s;
            const nm22 = a * s;
            ...
            if (tr >= 0.0) {
                ...
                if (nm00 >= nm11 and nm00 >= nm22) {
                    t = @sqrt(nm00 - (nm11 + nm22) + 1.0);
                    x = t * 0.5;
                    t = 0.5 / t;
                    y = (nm10 + nm01) * t;
                    z = (nm02 + nm20) * t;
            ...
            return Self.new(
                Self.fma(self.w, w, Self.fma(-self.x, x, Self.fma(-self.y, y, -self.z * z))),
                Self.fma(self.w, x, Self.fma(self.x, w, Self.fma(self.y, z, -self.z * y))),
                Self.fma(self.w, y, Self.fma(-self.x, z, Self.fma(self.y, w, self.z * x))),
                Self.fma(self.w, z, Self.fma(self.x, y, Self.fma(-self.y, x, self.z * w))),
            );
        }
